...
export default function OverduePayments() {
  ...
  const loadPayments = async () => {
    try {
      ...
      params.status = 'pending'; // Only get pending payments
      const response = await payments.getAll(params);
      
      if (response.success) {
        // Filter for overdue payments (past due date)
        const today = new Date();
        const overduePayments = response.data.payments.filter((payment: any) => {
          if (!payment.dueDate) return false;
          const dueDate = new Date(payment.dueDate);
          return dueDate < today;
        });
        setPaymentSchedules(overduePayments);
        setPagination({...response.data.pagination, total: overduePayments.length});
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && !error && paymentSchedules.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-destructive">Overdue Payment List</CardTitle>
          </CardHeader>
          <CardContent>
            {viewMode === 'cards' ? (
              <div className="grid gap-4">
                {paymentSchedules.map((schedule) => {
                  ...
                  return (
                    <Card key={schedule.id} className="hover:shadow-lg transition-shadow border-destructive/20">
                      <CardHeader>
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="flex gap-2 pt-2">
                          ...
                          {canManagePayments && (
                            <Button size="sm" variant="default" onClick={() => handleEditSchedule(schedule)}>
                  ...
                  {paymentSchedules.map((schedule) => {
                    ...
                    return (
                      <TableRow key={schedule.id} className="border-destructive/10">
                        ...
                        <TableCell>
                          <div className="text-destructive font-medium">{new Date(schedule.dueDate).toLocaleDateString()}</div>
                          {daysDiff !== null && (
      ...
      {!loading && !error && paymentSchedules.length === 0 && (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">All payments are on track!</p>
          </CardContent>
        </Card>
      )}
      ...
      <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Process Overdue Payment</DialogTitle>
          </DialogHeader>
          {editingSchedule && (
            <div className="space-y-4">
