...
export default function ApprovedApplications() {
  ...
  const filterHook = useApplicationFilters('approved');
  ...
  const handleExport = () => {
    exportApplications(filterHook.getExportParams(), 'approved_applications');
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">View approved applications</p>
        </div>
      ...
      <Card>
        <CardHeader><CardTitle>Approved Applications ({pagination.total})</CardTitle></CardHeader>
        <CardContent>
          {loading ? (
            ...
            <p className="text-muted-foreground text-center py-8">No approved applications found</p>
          ) : viewMode === 'cards' ? (
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-2 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <div className="text-sm text-muted-foreground"><span className="font-medium">Approved Amount:</span> ₹{(app.approvedAmount || app.requestedAmount).toLocaleString()}</div>
                      </div>
                    ...
                    <div className="flex flex-col gap-2 items-end">
                      <Badge variant="outline" className="bg-success/10 text-success border-success/20"><CheckCircle className="mr-1 h-3 w-3" />APPROVED</Badge>
                ...
                {applicationList.map((app) => (
                  <TableRow key={app._id}>
                    ...
                    <TableCell>
                      ...
                      <div className="text-sm font-medium mt-1">₹{(app.approvedAmount || app.requestedAmount).toLocaleString()}</div>
                    </TableCell>
